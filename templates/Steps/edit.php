...
<div class="p-8 text-lg" id="mainContent">
    ...
    <div x-data="{openTab: 0}">
        <div class="flex justify-start gap-4 my-5">
            <button @click="openTab = 1" :class="{ 'bg-slate-200 text-slate-900' : openTab === 1 }" class="px-4 py-2 text-white text-md bg-slate-700 hover:bg-slate-700/80 hover:no-underline rounded-lg">
                Add Existing Activity
            </button>

            <button @click="openTab = 2" :class="{ 'bg-slate-200 text-slate-900' : openTab === 2 }" class=" px-4 py-2 text-white text-md bg-slate-700 hover:bg-slate-700/80 hover:no-underline rounded-lg">
                Add New Activity
            </button>
            <button @click="openTab = 3" :class="{ 'bg-slate-200 text-slate-900' : openTab === 3 }" class=" px-4 py-2 text-white text-md bg-slate-700 hover:bg-slate-700/80  hover:no-underline rounded-lg">
                Edit Step Info
            </button>
            <button @click="openTab = 4" :class="{ 'bg-slate-200 text-slate-900' : openTab === 4 }" class=" px-4 py-2 text-white text-md bg-slate-700 hover:bg-slate-700/80  hover:no-underline rounded-lg">
                Add New Step
            </button>
        </div>
        ...
        <div class="max-w-prose">
            <div xcloak x-show="openTab === 1" @click.outside="openTab = 0" class="outline outline-1 outline-offset-2 outline-slate-500 p-6 my-3 rounded-md block">
                ...
                <form method="get" id="actfind" action="/activities/stepfind" class="my-2 flex justify-between gap-2">
                    ...
                    <button class="px-4 py-2 text-white text-md bg-slate-700 hover:bg-slate-700/80 hover:no-underline rounded-lg" type="submit">Search</button>
                </form>
            ...
            <div xcloak x-show="openTab === 2" @click.outside="openTab = 0" class="outline outline-1 outline-offset-2 outline-slate-500 p-6 my-3 rounded-md block">
                ...
                <?= $this->Form->button(__('Save Activity'), ['class' => 'px-4 py-2 text-white text-md bg-slate-700 hover:bg-slate-700/80 hover:no-underline rounded-lg mt-3']) ?>
                <?= $this->Form->end() ?>
            </div>
            ...
            <div xcloak x-show="openTab === 4" @click.outside="openTab = 0" class="outline outline-1 outline-offset-2 outline-slate-500 p-6 my-3 rounded-md block">
                ...
                <div class="mt-2"><?php echo $this->Form->control('description', ['class' => 'form-field', 'type' => 'textarea', 'label' => 'Objective']);
                                    ...
                                    ?></div>
                <?= $this->Form->button(__('Add Step'), ['class' => 'px-4 py-2 text-white text-md bg-slate-700 hover:bg-slate-700/80 hover:no-underline rounded-lg mt-3']) ?>
                <?= $this->Form->end() ?>
            </div>
        </div>
    </div>
            ...
            <div class="flex justify-start gap-4 items-center">
                <div class="basis-1/7 flex-none flex flex-col justify-center items-end text-sm">
                    <div class="">
                        ...
                        <button class="px-3 py-1 text-white text-md bg-slate-700 hover:bg-slate-700/80 hover:no-underline rounded-lg">Up <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-caret-up-fill inline-block" viewBox="0 0 16 16">
                    ...
                    <div class="">
                        ...
                        <?= $this->Form->control('step_id', ['type' => 'hidden', 'value' => $step->id]) ?>
                        <?= $this->Form->control('activity_id', ['type' => 'hidden', 'value' => $a->id]) ?>
                        <button class="px-3 py-1 text-white text-md bg-slate-700 hover:bg-slate-700/80 hover:no-underline rounded-lg mt-1">Down <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-caret-down-fill inline-block" viewBox="0 0 16 16">
                        ...
                        <div class="flex flex-row justify-between">
                            ...
                            <div class="bg-white inset-1 rounded-r-sm flex-1">
                                ...
                                <div class="p-3 text-lg">
                                    ...
                                    <button class="px-3 py-1 text-white text-sm bg-slate-700 hover:bg-slate-700/80 hover:no-underline rounded-lg mt-3">Save Context</button>
                                    <?= $this->Form->end() ?>
                                </div>
                            </div>
                        </div>
                ...
                <div class="basis-2/7 flex-none flex flex-col justify-center items-start text-sm">
                    ...
                    <div>
                        ...
                        <?= $this->Form->control('step_id', ['type' => 'hidden', 'value' => $step->id]) ?>
                        <?= $this->Form->control('activity_id', ['type' => 'hidden', 'value' => $a->id]) ?>
                        <?= $this->Form->button(__('Make Supplemental'), ['class' => 'px-3 py-1 text-white text-sm bg-slate-700 hover:bg-slate-700/80 hover:no-underline rounded-lg ']) ?>
                        <?= $this->Form->end() ?>
                    </div>
                    <div>
            ...
            <div class="flex justify-start gap-4 items-center">
                ...
                <div class="basis-2/7 flex-none flex flex-col justify-center items-start text-sm">
                    <div>
                        ...
                        <?= $this->Form->control('step_id', ['type' => 'hidden', 'value' => $step->id]) ?>
                        <?= $this->Form->control('activity_id', ['type' => 'hidden', 'value' => $a->id]) ?>
                        <?= $this->Form->button(__('Make Required'), ['class' => 'px-3 py-1 text-white text-sm bg-slate-700 hover:bg-slate-700/80 hover:no-underline rounded-lg ']) ?>
                        <?= $this->Form->end() ?>
                    </div>
                    <div>
                        <?= $this->Form->create(null, ['action' => '/activities-steps/delete/' . $a->_joinData->id, 'class' => 'inline-block']) ?>
